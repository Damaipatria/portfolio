      ...
      <div className="relative h-56 overflow-hidden">
        {!data?.confidential ? (
          <>
            ...
            <div className="lg:block hidden absolute inset-0 bg-black/60  opacity-0 group-hover:opacity-100 transition-opacity duration-400">
        ...
        <div className="absolute bottom-0 w-full py-1 px-2 bg-black/50 backdrop-blur-sm">
          <div className="flex items-center gap-2">
            <div className={`${data.status === "Production" ? "bg-green-400" : data.status === "Development" ? "bg-yellow-500" : ""} size-1.75 outline outline-white rounded-full`} />
            <p className="pt-0.5 text-xs text-gray-50 font-bold">{data.status}</p>
          </div>
        </div>
      </div>
      <div className="flex flex-col h-[calc(100%-14rem)] md:px-3 p-2">
        ...
        <div className="flex justify-between items-center mt-auto pt-2 px-0 border-t border-gray-400">
          <div>
            {!data?.confidential ? (
              <button
                className="lg:hidden flex items-center gap-1 py-1.5 px-3 text-sm text-gray-600 border rounded-sm cursor-pointer hover:bg-gray-800 hover:text-white active:bg-gray-950"
                onClick={() => window.open(data?.demoUrl, "_blank")}
              >
                <ExternalLink size={16} />
                <p className="font-bold">Kunjungi</p>
          ...
          <button
            className="flex items-center gap-1 py-1.5 px-3 text-sm text-gray-600 border rounded-sm cursor-pointer hover:bg-gray-800 hover:text-white active:bg-gray-950"
